      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < MAX) {
            ...
            g.addColorStop(0, `rgba(61,22,88,${alpha})`);
  ...
  return (
    <section id="hero" className="relative flex items-center overflow-hidden" style={{ minHeight: '100dvh' }}>
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute top-1/4 right-1/3 w-96 h-96 rounded-full blur-[120px]" style={{ background: 'rgba(61,22,88,0.07)' }} />
      </div>

      {/* Top accent line */}
      <div className="absolute top-0 left-0 right-0 h-1 z-10" style={{ background: 'linear-gradient(to right, transparent, #3D1658, #2A0F3E, transparent)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-20 pb-24 sm:pb-16 w-full">
        <div className="max-w-3xl">
          ...
          <div className="hero-stagger flex items-center gap-3 mb-6" style={{ animationDelay: '0.05s' }}>
            <div className="h-px w-10" style={{ background: 'linear-gradient(to right, #3D1658, #2A0F3E)' }} />
          ...
          <div className="hero-stagger flex flex-wrap gap-3" style={{ animationDelay: '0.60s' }}>
            {badgeLabels.map((label, i) => {
              ...
              return (
                <div key={label} className="flex items-center gap-2 backdrop-blur-sm rounded-full px-4 py-2" style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(61,22,88,0.25)' }}>
          ...
          <div className="hero-stagger flex gap-3 mt-6 md:hidden" style={{ animationDelay: '0.72s' }}>
            {stats.map((s) => (
              <div key={s.label} className="flex-1 text-center rounded-xl px-2 py-3" style={{ background: 'rgba(255,255,255,0.82)', border: '1px solid rgba(61,22,88,0.18)', boxShadow: '0 2px 12px rgba(61,22,88,0.08)' }}>
        ...
        <div className="absolute bottom-12 right-6 lg:right-16 hidden md:flex flex-col gap-3">
          {stats.map((s) => (
            <div key={s.label} className="text-right backdrop-blur rounded-xl px-5 py-3 min-w-[150px] transition-all hover:scale-105" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(61,22,88,0.2)', boxShadow: '0 4px 20px rgba(61,22,88,0.1)' }}>
  ...
  return (
    <section id="hero" className="relative flex items-center overflow-hidden" style={{ minHeight: '100dvh' }}>
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute top-1/4 right-1/3 w-96 h-96 rounded-full blur-[120px]" style={{ background: 'rgba(61,22,88,0.1)' }} />
        <div className="absolute bottom-1/3 left-1/4 w-64 h-64 rounded-full blur-[100px]" style={{ background: 'rgba(14,0,43,0.08)' }} />
      </div>

      <ParticleMesh />
      <div className="absolute top-0 left-0 right-0 h-px z-10" style={{ background: 'linear-gradient(to right, transparent, rgba(61,22,88,0.5), rgba(14,0,43,0.5), transparent)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-20 pb-24 sm:pb-16 w-full">
        <div className="max-w-3xl">
          ...
          <div className="hero-stagger flex flex-wrap gap-3" style={{ animationDelay: '0.60s' }}>
            {badgeLabels.map((label, i) => {
              const Icon = [Shield, Truck, FileText][i];
              return (
                <div key={label} className="flex items-center gap-2 backdrop-blur-sm rounded-full px-4 py-2" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(61,22,88,0.3)' }}>
          ...
          <div className="hero-stagger flex gap-3 mt-6 md:hidden" style={{ animationDelay: '0.72s' }}>
            {statItems.map((s) => (
              <div key={s.label} className="flex-1 text-center rounded-xl px-2 py-3" style={{ background: 'rgba(26,24,48,0.8)', border: '1px solid rgba(61,22,88,0.25)' }}>
                <div className="text-lg font-bold text-gradient-stats leading-none">{s.value}</div>
        ...
        <div className="absolute bottom-12 right-6 lg:right-16 hidden md:flex flex-col gap-3">
          {statItems.map((s) => (
            <div key={s.label} className="text-right backdrop-blur rounded-xl px-5 py-3 min-w-[150px] transition-all hover:scale-105" style={{ background: 'rgba(26,24,48,0.8)', border: '1px solid rgba(61,22,88,0.25)' }}>
              <div className="text-2xl font-bold text-gradient-stats">{s.value}</div>
